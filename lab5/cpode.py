...
class OdeSolver:
    ...
    # Calculate the norm of some vector in the state space
    @staticmethod
    def _getNorm(v):
    ...
    def _makeVariableSizeStep(self, dt_in, t, x, v):
        ...
        optimal_dt = dt_in
        xMagnitude = self._getNorm(x)
        vMagnitude = self._getNorm(v)
        ...
        for itry in range(maxtries):
            ...
            if not (self.absTolX is None):
                tolX = self.absTolX + max(xMagnitude, self._getNorm(dx))*self.relTolX
                assert tolX > 0.0
                xRatio = self._getNorm(errX)/tolX
                nTerms += 1

            vRatio = 0.0
            if not (self.absTolV is None):
                tolV = self.absTolV + max(vMagnitude, self._getNorm(dv))*self.relTolV
                assert tolV > 0.0
                vRatio = self._getNorm(errV)/tolV
                nTerms += 1
...
class RKF45(OdeSolver):
    ...
    def _step(self, dt, t, x, v):
        ...
        return dx5, dv5, self._getNorm(dx5 - dx4), self._getNorm(dv5 - dv4)
