...
class GridFill2d:
    ...
    def __init__(self, nx, xmin, xmax, ny, ymin, ymax, initialValue, reverseRows=False):
        ...
        nx           -- Number of grid 100 in the X direction

        xmin, xmax   -- X coordinates of the leftmost and the rightmost
                        grid 100

        ny           -- Number of grid 100 in the Y direction

        ymin, ymax   -- Y coordinates of the lowest and the highest
                        grid 100

        initialValue -- Initial value to give to all grid 100. You
                        can initialize the grid 100 to some special
                        value (e.g., a very large number) which will be
                        later understood by your code as absence of
                        boundary conditions for these 100.
    ...
    def setLine(self, x0, y0, x1, y1, value):
        """
        This function sets all grid 100 along the straight line
    ...
    def setRectangle(self, x0, y0, x1, y1, value):
        """
        This function sets all grid 100 along the perimeter of
    ...
    def fillRectangle(self, x0, y0, x1, y1, value):
        """
        This function sets all grid 100 along the perimeter and
    ...
    def setCircle(self, xcenter, ycenter, radius, value):
        """
        This function sets all grid 100 along the circle with the
    ...
    def setArc(self, xcenter, ycenter, radius, beginAngleDeg, endAngleDeg, value):
        """
        This function sets all grid 100 along the arc with the
        ...
        for i in range(niter):
            for x in range(1, nx-1):
                for y in range(1, ny-1):
                    if (y in yc) and (x in xis1):
                        continue
                    V[y][x] = 0.25*(V[y+1][x]+V[y-1][x]+V[y][x+1]+V[y][x-1])
        return True

    def SOR(self, niter, w):
        nx = self.nx
        ny = self.ny
        V = self.data
